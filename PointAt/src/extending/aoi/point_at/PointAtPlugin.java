 ...
 *
 */
public class PointAtPlugin implements Plugin {

	private LayoutWindow layout;
	private ObjectInfo pointer;
	private ObjectInfo pointee;
	public static final int numObjectsRequired = 2;
	...
	public void processMessage(int message, Object[] args) {
		...
		case Plugin.SCENE_WINDOW_CREATED:
			...
			BMenuItem menuItem = Translate.menuItem("Point At...", this, "pointAtMenuAction");
	...
	private void controlEnableDisable(BMenuItem menuItem) {
		...
		new PointAtMenuItemActivator(menuItem, layout).start();
	}
	...
	private void pointAtMenuAction(){
		...
		if(objects.size() == numObjectsRequired ){
			...
			// We need to know which object points at which, so
			// bring up a window for the user to tell us.
			ObjectInfo objA = (ObjectInfo) objects.toArray()[0];
			ObjectInfo objB = (ObjectInfo) objects.toArray()[1];

			setPointeeFromUserInput(objA, objB);

			for(ObjectInfo obj : objects){
				if(obj != this.pointee){ // don't point something at itself
					pointPointerAtPointee(this.pointer, undo);
				}
			}
			...
			System.err.println("ERROR: PointAtPlugin - number of selected objects not equal "+numObjectsRequired);
		}
	 ...
	 * @return
	 */
	private boolean specialPointerObject() {
		boolean ret = false;
	 ...
	 * @param objB
	 */
	private void setPointeeFromUserInput(ObjectInfo objA,
			ObjectInfo objB) {

		PointAtObjectSelectionWindow window = new PointAtObjectSelectionWindow(layout, objA, objB);

		pointer = window.getPointer();
